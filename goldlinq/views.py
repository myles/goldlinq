...
def gallery_list(page=1):
    object_list = Gallery.parse_directory(current_app.config["GALLERIES_PATH"])
    object_list.order_by("dt_published")

    paginator = Paginator(object_list, current_app.config["PER_PAGE"])
